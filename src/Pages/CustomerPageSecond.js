...
const CustomerPageSecond = () => {

    const [jobs, setJobs] = useState([]);
    const [allJobs, setAllJobs] = useState([]);

    const getAllJobs = async()=>{
        const response = await AlleJobberService.getAllJobs();
        setJobs(response);
        setAllJobs(response);
    }
    ...
    const getJobsById = async (bedrift) =>{
        const foundJobb = allJobs.find((jobb) => 
        jobb.bedrift === String(bedrift));
        if(foundJobb != null) {
            setJobs([foundJobb]);
        } else {
            setJobs([]);
            ...
            <div className="flex justify-center ">
                ...
                <div className="grid grid-rows-[1fr auto] h-full">
                    ...
                    <div className="row-span-1 flex flex-col-reverse">
                        <button
                        className="w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline-blue active:bg-blue-800 Menu"
                        onClick={resetJobs}
                        >
                        Tilbakestill
                        </button>
...
export default CustomerPageSecond;